...
// External Interrupt Number for STM32F101/103
//#define  BSP_INT_SRC_NBR                                 42
// Interrupt Number for STM32F107/105
...
void  BSP_IntDis (CPU_DATA  int_id)
{
    if (int_id < BSP_INT_SRC_NBR) {
        CPU_IntSrcDis(int_id + 16); // +16 是因为0～15中断是Cortex-M3内核使用中断源 从16开始是外设中断源
    }
}
...
void  BSP_IntEn (CPU_DATA  int_id)  // 在NVIC中运行相应中断
{
    if (int_id < BSP_INT_SRC_NBR) {
...
void  BSP_IntHandlerETHWakeUp     (void)  { BSP_IntHandler(BSP_INT_ETHWakeUp);          }
